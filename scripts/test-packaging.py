...
def test_a_certificate_of_the_wrong_kind_is_refused() -> None:
    ...
    every architecture, and `stapler` then failed with error 65 because there
    was no ticket to staple.

    A third check that failed open where the two above failed closed: same
    family, opposite direction, and this one costs a full universal build and a
    round trip to Apple before it says anything.
